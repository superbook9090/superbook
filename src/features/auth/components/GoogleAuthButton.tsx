...
export default function GoogleAuthButton({ callbackUrl }: GoogleAuthButtonProps) {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.98 }}
      className="w-full flex items-center justify-center py-3.5 px-4 bg-[var(--card-solid)] border-2 border-[var(--color-border)] rounded-xl hover:border-[var(--color-muted)] hover:bg-[var(--color-surface-muted)] transition-all text-sm font-medium"
    >
