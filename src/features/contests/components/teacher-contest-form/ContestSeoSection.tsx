  ...
  return (
    <div className="p-6 rounded-3xl bg-[var(--card-solid)] border border-[var(--border)] shadow-xs space-y-4">
      <div className="flex items-center gap-2 pb-3 border-b border-[var(--border)] text-sm font-bold text-[var(--color-foreground)]">
        ...
        <span>{t('contests.seoSettings')}</span>
      </div>

      <div className="space-y-4">
        <TextField
          label={t('contests.urlSlugOptional')}
          value={slug}
        ...
        <TextField
          label={t('contests.metaTitleOptional')}
          value={metaTitle}
          onChange={(e) => setMetaTitle(e.target.value)}
          placeholder={t('contests.metaTitlePlaceholder')}
          maxLength={70}
        />

        <div>
          <label className="block text-xs font-semibold text-[var(--color-foreground)] mb-1">
            {t('contests.metaDescriptionOptional')}
          </label>
          <textarea
            rows={2}
            value={metaDescription}
            onChange={(e) => setMetaDescription(e.target.value)}
            placeholder={t('contests.metaDescPlaceholder')}
            maxLength={180}
