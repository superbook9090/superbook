...
export const SEED_ARTICLES: SeedArticleData[] = [
  {
...
export async function seedEducationalArticles(preferredAuthorId?: string) {
  await dbConnect();

  // Find a suitable author
  let authorId = preferredAuthorId;
  if (!authorId || !mongoose.isValidObjectId(authorId)) {
    const defaultAuthor = await User.findOne({
      $or: [{ role: 'superadmin' }, { role: 'admin' }, { role: 'teacher' }],
    }).select('_id').lean();

    if (!defaultAuthor) {
      throw new Error('No admin or teacher user found to author the seeded articles.');
    }
    authorId = (defaultAuthor as { _id: mongoose.Types.ObjectId })._id.toString();
  }

  // 1. Unpublish all dummy test blogs
  const unpublishRes = await Blog.updateMany(
    {
      $or: [
  ...
  for (const art of SEED_ARTICLES) {
    ...
    const existing = await Blog.findOne({ slug: art.slug });
    ...
    if (existing) {
      await Blog.updateOne({ _id: existing._id }, { $set: doc });
      updated++;
    } else {
      await Blog.create({
        ...doc,
  ...
  // Invalidate redis cache for blogs
  try {
    await invalidatePattern('blogs:*');
  } catch {
    // Redis optional fallback
  }

  const totalPublished = await Blog.countDocuments({
    isPublished: true,
    title: { $not: /^test\d*/i },
  });
  ...
  return {
    success: true,
    unpublishedTestsCount: unpublishRes.modifiedCount,
    inserted,
    updated,
    totalPublished,
  };
